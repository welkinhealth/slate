...
Apps allow Welkin's customers to utilize tools that they already use to create a single integrated workflow within Welkin. Welkin supports showing UI built by 3rd parties within the Welkin experience. This UI extension is called an App in the Welkin ecosystem. Welkin loads these actions, once configured, into secured containers within the Welkin Care Staff interface.
...
## JWT auth claim
  claim = {
    'iss': client_id,
    'aud': audience,
    'exp': arrow.utcnow().replace(seconds=600).timestamp,
    'welkin_patient_id': patient_id,
    'welkin_worker_id': worker_id,
    'welkin_provider_id': provider_id,
  }
  jwt = jwt.encode(claim, client_secret, algorithm='HS256')
...
Example request:
POST
URL: https://www.example.com/test/app
Body: token=<jwt>
